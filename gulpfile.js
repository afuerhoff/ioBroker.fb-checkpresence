    ...
    for (const word in data) {
        if (data.hasOwnProperty(word)) {
            ...
            if (line) {
                ...
                line = line.replace(/<\/ /g, '</'); //add after translation a space is in the html tag e.g. "</ span>"
            }
            text += line + '},\n';
        }
    }
        ...
        if (fs.existsSync('./admin/i18n/en/translations.json')) {
            ...
            for (let l in languages) {
                ...
                for (let t in enTranslations) {
                    if (!existing[t]) {
                        //existing[t] = await translate(enTranslations[t], l, yandex); //original
                        const new_existing = await translate(enTranslations[t], l, yandex);
                        let lg = '';
                        for (let x in new_existing){
                            lg += new_existing[x][0];
                        }
                        existing[t] = lg;
                        await _sleep(200); //to prevent translation error
                    }
                }
