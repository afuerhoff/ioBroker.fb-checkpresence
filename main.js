...
class FbCheckpresence extends utils.Adapter {
    ...
    async resyncFbObjects(items) {
        try {
            ...
            if (items) {
                ...
                this.log.info('fb-devices successfully synchronized!');
            }
        } catch (error) {
    ...
    async onReady() {
        try {
            ...
            if (this.Fb.GETPATH != null && this.Fb.GETPATH == true && this.config.fbdevices == true) {
                ...
                // Start once if syncfbdevices is set to true
                if (this.config.syncfbdevices === true) {
                ...
                if (this.config.fbdevices && this.config.syncfbdevicesMaxDays > 0) {
                    ...
                    this.log.info(`resyncFbObjects scheduled every ${this.config.syncfbdevicesMaxDays} days`);
                }
            }
            ...
            for (let i = 0; i < hosts.length; i++) {
                ...
                if (this.Fb.GETMESHPATH != null && this.Fb.GETMESHPATH == true && enabledMeshInfo == true) {
                    if (mesh != null) {
                        ...
                        if (meshdevice != null) {
                            //host in the meshlist
                            await this.setStateChangedAsync(`fb-devices.${hostName}.meshstate`, {
                                val: true,
                                ack: true,
                            });
                            ...
                            for (const c in hostCh) {
                                ...
                                if (found == false) {
                                    ...
                                    if (
                                        Number.isInteger(
                                            parseInt(
                                                hostCh[c]._id.replace(`${this.namespace}.fb-devices.${hostName}.`, ''),
                                            ),
                                        )
                                    ) {
                                        ...
                                        await this.setStateChangedAsync(`${hostCh[c]._id}.cur_data_rate_rx`, {
                                            val: 0,
                                            ack: true,
                                        });
                                        await this.setStateChangedAsync(`${hostCh[c]._id}.cur_data_rate_tx`, {
                                            val: 0,
                                            ack: true,
                                        });
                                        await this.setStateChangedAsync(`${hostCh[c]._id}.rx_rcpi`, {
                                            val: 0,
                                            ack: true,
                                        });
                                        await this.setStateChangedAsync(`${hostCh[c]._id}.link`, {
                                            val: '',
                                            ack: true,
                                        });
                                    }
                            ...
                            for (let ni = 0; ni < meshdevice['node_interfaces'].length; ni++) {
                                ...
                                await this.setStateChangedAsync(`fb-devices.${hostName}.${ifNewName}.name`, {
                                    val: ifName,
                                    ack: true,
                                });

                                if (nInterface['node_links'].length > 0) {
                                    ...
                                    for (let nl = 0; nl < nInterface['node_links'].length; nl++) {
                                        ...
                                        if (nodelinks['state'] == 'CONNECTED') {
                                            if (nodelinks['node_1_uid'] != meshdevice['uid']) {
                                                //Top connection
                                                ...
                                                link =
                                                    link == ''
                                                        ...
                                                        : (link += `,${node1['device_name']}`);
                                                const intf = node1.node_interfaces.find(
                                                    el => el.uid === nodelinks['node_interface_1_uid'],
                                                );
                                                ifType += ` ${intf['name']}`;
                                            }
                                            if (nodelinks['node_2_uid'] != meshdevice['uid']) {
                                                //Down connection
                                        ...
                                        await this.setStateChangedAsync(`fb-devices.${hostName}.${ifNewName}.rx_rcpi`, {
                                            val: Math.round(nodelinks['rx_rcpi']),
                                            ack: true,
                                        });
                                        await this.setStateChangedAsync(
                                            `fb-devices.${hostName}.${ifNewName}.cur_data_rate_rx`,
                                            { val: data_rate_rx, ack: true },
                                        );
                                        await this.setStateChangedAsync(
                                            `fb-devices.${hostName}.${ifNewName}.cur_data_rate_tx`,
                                            { val: data_rate_tx, ack: true },
                                        );
                                    }
                                    await this.setStateChangedAsync(`fb-devices.${hostName}.${ifNewName}.link`, {
                                        val: link,
                                        ack: true,
                                    });
                                } else {
                                ...
                                await this.setStateChangedAsync(`fb-devices.${hostName}.${ifNewName}.type`, {
                                    val: ifType,
                                    ack: true,
                                });
                            }
                        } else {
                            //host not in meshlist
                            for (const c in hostCh) {
    ...
    async getWlBlInfo() {
        try {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                if (wlFound == false && items[i] != null) {
                    //blacklist
                    //deviceType = 'blacklist';
                    if (items[i]['X_AVM-DE_Guest'] == false) {
                        htmlBlRow += this.createHTMLTableRow([
                            items[i]['HostName'],
                            items[i]['IPAddress'],
                            items[i]['MACAddress'],
                        ]);
                        ...
                        jsonBlRow.push({
                            ...
                            'IP-Address': items[i]['IPAddress'],
                            'MAC-Address': items[i]['MACAddress'],
                        });
                        blCnt += 1;
                    }
                }
                if (wlFound == true) {
                    ...
                    htmlWlRow += this.createHTMLTableRow([
                        items[i]['HostName'],
                        items[i]['IPAddress'],
                        items[i]['MACAddress'],
                    ]);
                ...
                if (host && host.length > 0) {
                    ...
                    if (count == macs.length) {
                        await this.setStateChangedAsync(`fb-devices.${hostName}.whitelist`, {
                            val: true,
                            ack: true,
                        });
                        await this.setStateChangedAsync(`fb-devices.${hostName}.blacklist`, {
                            val: false,
                            ack: true,
                        });
                    } else {
            ...
            if (blCnt > 0) {
                if (this.config.compatibility == true) {
                    await this.setStateChangedAsync('blacklist', { val: true, ack: true });
                }
                await this.setStateChangedAsync('blacklist.presence', { val: true, ack: true });
            } else {
                if (this.config.compatibility == true) {
                    await this.setStateChangedAsync('blacklist', { val: false, ack: true });
                }
    ...
    async getDeviceInfo() {
        try {
            ...
            for (let i = 0; i < hosts.length; i++) {
                ...
                htmlFbDevices += this.createHTMLTableRow([
                    hosts[i]['hn'],
                    hosts[i]['ip'],
                    hosts[i]['mac'],
                    hosts[i]['active'],
                ...
                if (this.config.fbdevices === true && this.enabled == true) {
                    if (hostName != '') {
                        await this.setStateChangedAsync(`fb-devices.${hostName}.devicename`, {
                            val: hostName,
                            ack: true,
                        });
                        await this.setStateChangedAsync(`fb-devices.${hostName}.macaddress`, {
                            val: mac,
                            ack: true,
                        });
                        await this.setStateChangedAsync(`fb-devices.${hostName}.ipaddress`, {
                            val: ip,
                            ack: true,
                        });
                        await this.setStateChangedAsync(`fb-devices.${hostName}.active`, {
                            val: hosts[i]['active'],
                            ack: true,
                        });
                        ...
                        await this.setStateChangedAsync(`fb-devices.${hostName}.speed`, {
                            val: parseInt(hosts[i]['speed']),
                            ack: true,
                        });
                        await this.setStateChangedAsync(`fb-devices.${hostName}.guest`, {
                            val: hosts[i]['guest'],
                            ack: true,
                        });
                        await this.setStateChangedAsync(`fb-devices.${hostName}.vpn`, { val: vpn, ack: true });
                        if (hosts[i]['data'] != null) {
                            await this.setStateChangedAsync(`fb-devices.${hostName}.disabled`, {
                                val: hosts[i]['data']['X_AVM-DE_Disallow'] == 0 ? false : true,
                                ack: true,
                            });
                        }
                    } else {
            ...
            if (this.config.fbdevices === true && this.enabled == true) {
                ...
                await this.setStateChangedAsync('fb-devices.jsonActive', {
                    val: JSON.stringify(jsonFbDevActive),
                    ack: true,
                });
                await this.setStateChangedAsync('fb-devices.jsonInactive', {
                    val: JSON.stringify(jsonFbDevInactive),
                    ack: true,
                });
                await this.setStateChangedAsync('fb-devices.html', { val: htmlFbDevices, ack: true });
                await this.setStateChangedAsync('fb-devices.jsonActiveVPN', {
                    val: JSON.stringify(jsonFbDevActiveVPN),
                    ack: true,
                });
            }
            if (this.config.guestinfo === true && this.enabled == true) {
                ...
                if (this.config.compatibility == true) {
                    await this.setStateChangedAsync('guest', { val: val, ack: true });
                }
            }
    ...
    async getActive(memberRow) {
        ...
        try {
            ...
            let mesg = this.suppressArr.filter(function (x) {
                if (
                    x.name === memberRow.familymember &&
                    x.mac === memberRow.macaddress &&
                    x.ip === memberRow.ipaddress &&
                    x.hostname === memberRow.devicename
                ) {
                    return x;
                }
            });
                        ...
                        if (mac != '') {
                            ...
                            if (host && host.length > 0) {
                                ...
                                if (mesg[0].suppress === false) {
                                    ...
                                    throw new Warn(
                                        `The configured mac-address for member ${
                                            member
                                        } was not found in the fritzbox. Please insert a valid mac-address!`,
                                    );
                                }
                            }
                        } else {
                            if (mesg[0].suppress === false) {
                                ...
                                throw new Warn(
                                    `The configured mac-address for member ${
                                        member
                                    } is empty. Please insert a valid mac-address!`,
                                );
                            }
                        }
                        break;
                    case 'IP':
                        if (ip != '') {
                            ...
                            if (mesg[0].suppress === false) {
                                ...
                                throw new Warn(
                                    `The configured ip-address for ${
                                        member
                                    } is empty. Please insert a valid ip-address!`,
                                );
                            }
                        }
                        break;
                        ...
                        if (deviceName != '') {
                            ...
                            if (host && host.length > 0) {
                                ...
                                if (mesg[0].suppress === false) {
                                    ...
                                    throw Error(
                                        `The configured hostname for member ${
                                            member
                                        } was not found in the fritzbox. Please insert a valid hostname!`,
                                    );
                                }
                            }
                        } else {
                            if (mesg[0].suppress === false) {
                                ...
                                throw Error(
                                    `The configured hostname for ${member} is empty. Please insert a valid hostname!`,
                                );
                            }
                        }
                        break;
                    ...
                    case 'MAC':
                        if (mac != '') {
                            const soapResult = await this.Fb.soapAction(
                                ...
                                [[1, 'NewMACAddress', memberRow.macaddress]],
                            );
                            if (soapResult) {
                    ...
                    case 'IP':
                        if (this.Fb.GETBYIP == true && ip != '') {
                            const soapResult = await this.Fb.soapAction(
                                ...
                                [[1, 'NewIPAddress', memberRow.ipaddress]],
                            );
                            if (soapResult) {
                                active = soapResult['NewActive'] == 1 ? true : false;
                            }
    ...
    async calcMemberAttributesNew(memberRow, index, newActive, dnow, presence) {
        ...
        if (this.config.compatibility === true) {
            ...
            memberPath =
                memberRow.group == '' ? `familyMembers.${member}` : `familyMembers.${memberRow.group}.${member}`;
            dPoint = await this.getObjectAsync(`${this.namespace}` + `.${memberPath}.presence`);
            historyPath =
                memberRow.group == ''
                    ? `familyMembers.${member}.presence`
                    : `familyMembers.${memberRow.group}.${member}.presence`;
        }
        ...
        if (memberRow.group == '' && this.config.compatibility === true) {
            await this.setStateChangedAsync(memberPath, { val: newActive, ack: true });
        }
            ...
            if (dPoint.common.custom != undefined && dPoint.common.custom[this.config.history].enabled == true) {
                ...
                if (!histData) {
                    ...
                    for (let ih = 0; ih < historyArrFiltered.length; ih++) {
                        ...
                        htmlHistory += this.createHTMLTableRow([
                            historyArrFiltered[ih].val
                                ? '<div class="mdui-green-bg mdui-state mdui-card">anwesend</div>'
                                : '<div class="mdui-red-bg mdui-state mdui-card">abwesend</div>',
                            dateFormat(t2, this.config.dateformat),
                        ]);
                        jsonHistory.push({
                            Active: historyArrFiltered[ih].val,
                            Date: dateFormat(new Date(historyArrFiltered[ih].ts), this.config.dateformat),
                        });
                    ...
                    for (let ih = 0; ih < historyArrFiltered.length; ih++) {
                        ...
                        if (wert === true && comming === null) {
                            ...
                            if (flagBreak === false) {
                                await this.setStateChangedAsync(`${memberPath}.present.since`, {
                                    val: Math.round(dt / 60 / 1000),
                                    ack: true,
                                });
                                await this.setStateChangedAsync(`${memberPath}.absent.since`, {
                                    val: Math.round(0 / 60 / 1000),
                                    ack: true,
                                });
                                flagBreak = true;
                            }
                    ...
                    await this.setStateChangedAsync(`${memberPath}.present.sum_day`, {
                        val: Math.round(pr / 60 / 1000),
                        ack: true,
                    });
                    await this.setStateChangedAsync(`${memberPath}.absent.sum_day`, {
                        val: Math.round(ab / 60 / 1000),
                        ack: true,
                    });
                    htmlHistory += this.HTML_END;
                    await this.setStateChangedAsync(`${memberPath}.history`, {
                    ...
                    this.jsonTab.push({
                        Name: member,
                        Active: newActive,
                        Kommt: dateFormat(comming, this.config.dateformat),
                        Geht: dateFormat(going, this.config.dateformat),
                    });
                    this.htmlTab += this.createHTMLTableRow([
                        member,
                        newActive
                            ? '<div class="mdui-green-bg mdui-state mdui-card">anwesend</div>'
                            : '<div class="mdui-red-bg mdui-state mdui-card">abwesend</div>',
                        dateFormat(comming, this.config.dateformat),
                        dateFormat(going, this.config.dateformat),
                    ]);
                }
            }
        } else {
            this.log.warn(
                'New filter function: History is not enabled! Function was not used! Some datapoints were not calculated!',
            );
        }
    }
    ...
    async calcMemberAttributes(memberRow, index, newActive, dnow, presence) {
        try {
            ...
            if (curVal && curVal.val != null) {
                ...
                if (newActive == true) {
                    ...
                    if (curVal.val == null) {
                        ...
                        if (memberPath.group == '' && this.config.compatibility === true) {
                            await this.setStateChangedAsync(memberPath, { val: true, ack: true });
                        }
                    ...
                    if (curVal.val == null) {
                        ...
                        if (memberPath.group == '' && this.config.compatibility === true) {
                            await this.setStateChangedAsync(memberPath, { val: false, ack: true });
                        }
            ...
            if (presence != null) {
                ...
                if (this.config.history != '' && this.historyAlive.val === true) {
                    if (
                        dPoint.common.custom != undefined &&
                        dPoint.common.custom[this.config.history].enabled == true
                    ) {
                        try {
                            const result = await this.getHistoryTable(this, memb, historyPath, start, end);
                            if (!result) {
                                throw Error(`Can not get history items of member ${memb}`);
                            }
                            let htmlHistory = this.HTML_HISTORY;
                            const jsonHistory = [];
                            ...
                            for (let iv = 0; iv < result.result.length; iv++) {
                                ...
                                if (result.result[0].ts < result.result[result.result.length - 1].ts) {
                                    //Workaround for history sorting behaviour
                                    i = iv;
                                } else {
    ...
    async getMemberPropertiesFromFBDevices(memberRow) {
        try {
            ...
            const memberPath = this.config.compatibility
                ? group === ''
                    ? familymember
                    : `familyMembers.${group}.${familymember}`
                : group === ''
            ...
            //speed property
            const speed = items.length > 0 ? (items.find(item => item.active === '1') || items[0]).speed : 0;
    ...
    async checkPresence(trigger) {
        try {
            ...
            if (this.Fb && this.Fb.GETPATH === true) {
                //use of device list for presence if supported
                await this.Fb.getDeviceList();
                await this.getAllFbObjects();
            }
            ...
            for (let m = 0; m < membersFiltered.length; m++) {
                ...
                if (
                    activeNew === false &&
                    activeOld != activeNew &&
                    memberRow.usefilter === true &&
                    !this.config.newfilter
                ) {
                    filteringNeeded.push({
                        oldVal: activeOld,
                        newVal: activeNew,
                        member: member,
            ...
            if (filteringNeeded.length > 0) {
                ...
                for (let f = 0; f < filteringNeeded.length; f++) {
                    //loop over family members which are false
                    if (this.enabled == false) {
                        break;
                    } //cancel if disabled over unload
                    const memberRow = filteringNeeded[f].memberRow; //Row from family members table
                    const activeNew = await this.getActive(memberRow);
                    const ind = memberValues.findIndex(
                        x =>
                            x.member == memberRow.familymember &&
                            JSON.stringify(x.memberRow) == JSON.stringify(memberRow),
                    );
                    if (activeNew === false) {
                        memberValues[ind].newVal = activeNew;
                    }
                }
            }
            ...
            for (let g = 0; g < familyGroups.length; g++) {
                ...
                for (let k = 0; k < groupMembers.length; k++) {
                    //loop over enabled family members
                    if (this.enabled === false) {
                        break;
                    } //cancel if disabled over unload
                    const memberRow = groupMembers[k].memberRow; //Row from family members table
                    if (this.Fb && this.Fb.GETBYMAC == true) {
                        //member enabled in configuration settings and service is supported
                        const newActive = groupMembers[k].newVal;
                        if (trigger === true) {
                            this.log.info(`triggerPresence: State ${memberRow.familymember} ${newActive}`);
                        }
                        if (newActive != null && !this.config.newfilter) {
                            await this.calcMemberAttributes(memberRow, k, newActive, dnow, presence);
                        }
                        if (newActive != null && this.config.newfilter) {
                            await this.calcMemberAttributesNew(memberRow, k, newActive, dnow, presence);
                        }
                        if (newActive != null) {
                            this.getMemberPropertiesFromFBDevices(memberRow);
                        }
                    }
                }
                if (this.enabled == false) {
                ...
                await this.setStateChangedAsync(`${memberPath}absentMembers`, {
                    val: presence.absentMembers,
                    ack: true,
                });
                await this.setStateChangedAsync(`${memberPath}presentMembers`, {
                    val: presence.presentMembers,
                    ack: true,
                });
