...
async function _enableHistory(that, cfg, group, familyMember) {
    try {
        let alias = '';
        ...
        if (result[`${that.namespace}` + '.' + memberPath] != undefined && result[`${that.namespace}` + '.' + memberPath].aliasId != undefined){
            alias = result[`${that.namespace}` + '.' + memberPath].aliasId;
        }
        if (result[`${that.namespace}` + '.' + memberPath] != undefined && result[`${that.namespace}` + '.' + memberPath].enabled == true){
            return true;
        }else{
...
async function createGlobalObjects(that, adapterStates, table, tableGuest, enabled) {
    try {
        ...
        that.log.warn('The state "guest" will no longer exist in a future version. Please use "guest.presence" instead!');
        that.log.warn('The state "blacklist" will no longer exist in a future version. Please use "blacklist.presence" instead!');
        that.log.warn('The state "activeDevices" will no longer exist in a future version. Please use "fb-devices.active" instead!');
        that.log.warn('The state "devices" will no longer exist in a future version. Please use "fb-devices.count" instead!');
...
async function createMemberObjects(that, membersFiltered, familyGroups, adapterStates, memberStates, cfg, table, enabled) {
    try {
            ...
            for (let k = 0; k < membersFiltered.length; k++) {
                ...
                if (group == '' && that.config.compatibility === true){
                    opt = [
                        ...
                        [group + member + '.going', 'state', 'going', 'string', 'date', new Date(), true, false, 'time when you leaving the home'],
                        [group + member + '.comming', 'state', 'comming', 'string', 'date', new Date(), true, false, 'time when you arriving at home'],
                        [group + member + '.speed', 'state', member + '.speed', 'string', 'value', '', true, false, 'Speed of the device'],
                        [group + member + '.absent.since', 'state', member + '.absent.since', 'string', 'value', '0', true, false, 'absent since'],
                        [group + member + '.present.since', 'state', member + '.present.since', 'string', 'value', '0', true, false, 'present since'],
                        [group + member + '.absent.sum_day', 'state', member + '.absent.sum_day', 'string', 'value', '0', true, false, 'how long absent per day'],
                        [group + member + '.present.sum_day', 'state', member + '.present.sum_day', 'string', 'value', '0', true, false, 'how long present per day']
                    ];
                    that.log.warn('The state "fb-checkpresence.0.' + member + '" will no longer exist in a future version. Please use "fb-checkpresence.0.' + member + '.presence" instead!');
                }
                if (group == '' && that.config.compatibility === false){
                    opt = [
                        ...
                        ['familyMembers.' + group + member + '.going', 'state', 'going', 'string', 'date', new Date(), true, false, 'time when you leaving the home'],
                        ['familyMembers.' + group + member + '.comming', 'state', 'comming', 'string', 'date', new Date(), true, false, 'time when you arriving at home'],
                        ['familyMembers.' + group + member + '.speed', 'state', member + '.speed', 'string', 'value', '', true, false, 'Speed of the device'],
                        ['familyMembers.' + group + member + '.absent.since', 'state', member + '.absent.since', 'string', 'value', '0', true, false, 'absent since'],
                        ['familyMembers.' + group + member + '.present.since', 'state', member + '.present.since', 'string', 'value', '0', true, false, 'present since'],
                        ['familyMembers.' + group + member + '.absent.sum_day', 'state', member + '.absent.sum_day', 'string', 'value', '0', true, false, 'how long absent per day'],
                        ['familyMembers.' + group + member + '.present.sum_day', 'state', member + '.present.sum_day', 'string', 'value', '0', true, false, 'how long present per day']
                    ];
                }
                if (group != ''){
                    opt = [
                ...
                if (cfg.history != ''){
                    await _enableHistory(that, cfg, group, member);
                }else{
