...
async function createGlobalObjects(that, adapterStates, table, tableGuest, enabled) {
    try {
        ...
        let optDev = [
            ...
            ['fb-devices.jsonActiveVPN', 'state', 'fb-devices.jsonActiveVPN', 'string', 'json', '[]', true, false, 'fritzbox active vpn device list json'],
        ];
        if(that.config.compatibility === true){
        ...
        if (that.config.extip === true) opt = opt.concat(optExtIp);
            ...
            for (let k = 0; k < membersFiltered.length; k++) {
                ...
                if (group == '' && that.config.compatibility === true){
                    opt = [
                        ...
                        [group + member + '.link', 'state', member + '.link', 'string', 'value', '', true, false, 'Link of the device', ''],
                    ...
                    if (that.config.newfilter == true){
                        opt = opt.concat([[group + member + '.presenceFiltered', 'state', member + '.presenceFiltered', 'boolean', 'indicator', false, true, false, 'state of the family member']]);
                    }
                ...
                if (group == '' && that.config.compatibility === false){
                    opt = [
                        ...
                        ['familyMembers.' + group + member + '.link', 'state', member + '.link', 'string', 'value', '', true, false, 'Link of the device', ''],
                    ...
                    if (that.config.newfilter == true){
                        opt = opt.concat([['familyMembers.' + group + member + '.presenceFiltered', 'state', member + '.presenceFiltered', 'boolean', 'indicator', false, true, false, 'state of the family member']]);
                    }
                }
                if (group != ''){
                    opt = [
        ...
        if (that.config.fbdevices == true){
            for (let i = 0; i < hosts.length; i++) {
                ...
                const opt = [
                    ...
                    ['fb-devices.' + hostName + '.active', 'state', hostName + '.active', 'boolean', 'indicator.reachable', false, true, false, 'State of the device'],
                    ...
                    ['fb-devices.' + hostName + '.vpn', 'state', hostName + '.vpn', 'boolean', 'indicator', false, true, false, 'VPN connection'],
                    ['fb-devices.' + hostName + '.devicename', 'state', hostName + '.devicename', 'string', 'info.name', '', true, false, 'Name of the device']
                ];
