...
async function createGlobalObjects(that, adapterStates, table, tableGuest, enabled) {
    try {
        const optGeneral = [
            ...
            ['reconnect', 'state', 'reconnect', 'boolean', 'indicator', false, true, true, 'Reconnect Fritzbox'],
        ...
        if (that.config.fbdevices == true) opt = opt.concat(optDev, optGuest);
...
async function createFbDeviceObjects(that, adapterStates, hosts, enabled) { 
    try {
        if (hosts == null) throw('createFbDeviceObjects: hosts = null');
