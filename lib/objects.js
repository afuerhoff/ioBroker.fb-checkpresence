                ...
                if (!err && obj) {
                    that.extendObject(id, {
                        ...
                        if (!err && obj){
                            ...
                            reject('extendObject ' + id + ' ' + JSON.stringify(err));
                        }
                    });
                }else{
...
async function createGlobalObjects(that, table, tableGuest, enabled) {
    try {
        ...
        await _createObjects('createGlobalObjects', that, opt, enabled);
        that.log.info('createGlobalObjects finished successfully');
        return true;
    } catch (error) {
        that.log.error('createGlobalObjects: ' + JSON.stringify(error));
        return false;
    }
}
...
async function createMemberObjects(that, cfg, table, enabled) {
    try {
        if (cfg.members.length == 0) {
            ...
            for (let k = 0; k < cfg.members.length; k++) {
                ...
                if (memberRow.enabled == true){
                    ...
                    await _createObjects('createMemberObjects', that, opt, enabled);
                    count++;
                    if (cfg.history != ''){
        ...
        that.log.warn('createMemberObjects: ' + JSON.stringify(error));
        return false;      
    }
}

async function createFbDeviceObjectsNew(that, hosts, enabled) { 
    try {
        ...
        for (let i = 0; i < hosts.length; i++) {
            ...
            await _createObjects('createFbDeviceObjects', that, opt, enabled);
            count++;
            if (length == count) {
                return true;
            }
        ...
        that.log.warn('createFbDeviceObjects: ' + JSON.stringify(error));
        return false;    
    }
}
...
async function createFbDeviceObjects(that, items, enabled) { 
    try {
        if (!items) throw('createFbDeviceObjects: items = null');
        let count=0;
...
async function createMeshObjects(that, items, channel, enabled) { 
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            await _createObjects('createMeshObjects', that, opt, enabled); //.then(function(result){
            count++;
            if (length == count) {
                return true;
            }
        ...
        that.log.warn('createMeshObjects: ' + JSON.stringify(error));
        return false;      
    }
}
...
module.exports = {
    ...
    createFbDeviceObjectsNew: createFbDeviceObjectsNew,
