            ...
            for (let k = 0; k < membersFiltered.length; k++) {
                ...
                if (group == '' && that.config.compatibility === true){
                    opt = [
                        ...
                        [group + member + '.absent.since', 'state', member + '.absent.since', 'number', 'value', '0', true, false, 'absent since'],
                        [group + member + '.present.since', 'state', member + '.present.since', 'number', 'value', '0', true, false, 'present since'],
                        [group + member + '.absent.sum_day', 'state', member + '.absent.sum_day', 'number', 'value', '0', true, false, 'how long absent per day'],
                        [group + member + '.present.sum_day', 'state', member + '.present.sum_day', 'number', 'value', '0', true, false, 'how long present per day']
                    ];
