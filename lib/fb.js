...
class Fb {
        ...
        try {
            ...
            if (service) {
                ...
                if (response.status == 200){
                    ...
                    throw ('Can not find action ' + serviceId + '-' + action + ' ' + JSON.stringify(response));                        
                }
            }else{
                throw ('Can not find service file ' + serviceId);                        
            }
        } catch (err) {
            gthis.log.error('chkService: ' + JSON.stringify(err));
            return false;               
        }
    }
    ...
    async connectionCheck() {
        try {
            ...
            gthis.log.error('connectionCheck: ' + JSON.stringify(err));
            return false;            
        }
    }
