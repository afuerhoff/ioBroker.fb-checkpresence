...
class Fb {
    ...
    async _getAccessRights(){
        try {         
            ...
            //throw new exceptionSoapCommand('getAccessRights', error.message);
            //this.errorHandler(this.adapter, error, 'getAccessRights: ');
        }
    }
    ...
    async _chkService(action, serviceId, dp, neededAccessRights){
        try {
            ...
            //this.errorHandler(this.adapter, error, 'checkService: ');
            throw new exceptionFb('checkService', error.name + ' ' + error.message);
            //throw 'checkService:' + ' ' + error;
            //this.adapter.setState('info.' + dp, { val: false, ack: true });
            //this.adapter.log.warn('chkService: ' + JSON.stringify(err));
            //return false;               
        }
    }
    ...
    async _getWANAccessType(){
        try {
            ...
            //throw 'getWANAccessType:' + ' ' + error;
            //this.errorHandler(this.adapter, error, 'getWANAccessType: ');
        }        
    }
    ...
    async getExtIp(){
        try {
            ...
            let soapResult = {data: null};
            //DSL, Ethernet, X_AVM-DE_Fiber, X_AVMDE_UMTS, X_AVM-DE_Cable, X_AVM-DE_LTE
            if(this.GETEXTIP && this.GETEXTIP == true && (this.accessType == 'DSL' || this.accessType == 'X_AVM-DE_Cable' || this.accessType == 'X_AVM-DE_LTE' || this.accessType == 'X_AVMDE_UMTS' || this.accessType == 'X_AVM-DE_Fiber')){
                await this.soapAction('/upnp/control/wanpppconn1', 'urn:dslforum-org:service:' + 'WANPPPConnection:1', 'GetInfo', null, soapResult);
            }
            if (this.GETEXTIPBYIP && this.GETEXTIPBYIP == true && this.accessType == 'Ethernet') {
            ...
            //const ip = extIp.resultData['NewExternalIPAddress'];
            if (soapResult && soapResult.data){
                const ip = soapResult.data['NewExternalIPAddress'];
                //this.adapter.log.debug('external IP1: ' + ip);
                //this.adapter.setStateIfNotEqual('info.extIp', { val: ip, ack: true });
                soapResult.data = null;
                soapResult = null;
                return ip; 
            }else{
            ...
            //throw new exceptionFb('getExtIp', error.name + ' ' + error.message);
            //throw 'getExtIp:' + ' ' + error;
            //this.errorHandler(this.adapter, error, 'getExtIp: ');
            return '';
        }
    }
    ...
    //Get status of guest wlan 
    async getGuestQR(id, en){
        try {
            let soapResult2 = {data: null};
            if (this.WLAN3GETSECKEY && this.WLAN3GETSECKEY === true && en === true){
                ...
                if (soapResult2 && soapResult2.data){
                    ...
                    const WPA = this.beaconType == '11i' ? 'WPA2' : this.beaconType;
            ...
            return;
            //throw new exceptionFb('getGuestQR', error.name + ' ' + error.message);
            //throw new Error(error);
        }
    }
