...
class Fb {
    ...
    async _getAccessRights(){
        try {            
            const currentUser = await this.soapAction(this, '/upnp/control/lanconfigsecurity', urn + 'LANConfigSecurity:1', 'X_AVM-DE_GetCurrentUser', null);
            if (currentUser && currentUser.result === true && currentUser.resultData){
                const result = await xml2jsP.parseStringPromise(currentUser.resultData['NewX_AVM-DE_CurrentUserRights'], {explicitArray: false});
                if (result && result.rights && result.rights.path){
                    for(let i=0; i< result.rights.path.length; i++){
                        ...
                        //gthis.log.info(result.rights.path[i] + ': ' + result.rights.access[i]);
                    }
                    return true;
                }
            }
        } catch (error) {
            gthis.log.error('_getAccessRights' + JSON.stringify(error));
            return false;
        }
    }
    ...
    async _checkServices(){
        try {
            ...
            if (this.GETEXTIP == false) this.GETEXTIP = await this._chkService('GetInfo', 'WANIPConnection1', 'GetInfo', ['C']);
                ...
                if (response && response.status == 200){
                    ...
                    if (found == -1){
                        ...
                        if (supported === true){
                            ...
                            throw('service ' + serviceId + '-' + action + ' is supported but access right not ok! Feature is deactivated!');
                        }
                        return true;
                    }
                }else{
    ...
    async _sendWithRetry(url, timeout, start, message) {
        ...
        try {
            ...
            if (this.currentRetry < MAX_RETRY) {
                this.currentRetry++;
                gthis.log.debug(message + ': Retrying ' + this.currentRetry);
                return await this._sendWithRetry(url, timeout, this.currentRetry, message);
            } else {
                this.currentRetry = 0;
                gthis.log.warn(message + ': Retried ' + MAX_RETRY + ' times but still failed -> ' + JSON.stringify(error));
                return null;
            }   
        }
    }
    ...
    async getDeviceList(){
        try {
            ...
            if (hostPath && hostPath.result == true){
                ...
                const response = await this._sendWithRetry(url, 10000, 0, 'getDeviceList');
                ...
                if (response && response.status == 200 && response.data) {
                    ...
                    if (deviceList) {
                        ...
                        gthis.setState('fb-devices.count', { val: deviceList['List']['Item'].length, ack: true });
                        const inActiveHosts = deviceList['List']['Item'].filter(host => host.Active == '0');
                        gthis.setState('fb-devices.inactive', { val: inActiveHosts.length, ack: true });
                        const activeHosts = deviceList['List']['Item'].filter(host => host.Active == '1');
                        gthis.setState('fb-devices.active', { val: activeHosts.length, ack: true });
                        gthis.setState('activeDevices', { val: activeHosts.length, ack: true });
                        this.deviceList = deviceList['List']['Item'];
                        return deviceList['List']['Item'];
                    }else{
                        throw ('Can not parse response.data ' + JSON.stringify(response.data));                        
                    }
                }else{
    ...
    async getMeshList(){
        try {
            ...
            if (meshPath && meshPath.result == true){
                ...
                let response = null;
                response = await this._sendWithRetry(url, 10000, 0, 'getMeshList');
                if (response != null && response.status == 200 && response.data) {
                    ...
                    throw ('Can not read meshlist ' + JSON.stringify(meshPath));                        
                }
            }else{
                throw ('Can not read meshlist path');                        
            }
        } catch (err) {
    ...
    async _getWANAccessType(){
        try {
            if (this.GETACCESSTYPE && this.GETACCESSTYPE == true){
                const accessType = await this.soapAction(this, '/upnp/control/wancommonifconfig1', 'urn:dslforum-org:service:WANCommonInterfaceConfig:1', 'GetCommonLinkProperties', null, true);
                if (accessType != null && accessType.result == true){
                    this.accessType = accessType.resultData['NewWANAccessType'];
                    return true;
                }else{
                    throw('Can not read access type');                 
                }
            }
        } catch (error) {
            gthis.log.error('getWANAccessType: ' + JSON.stringify(error));
            return false;
        }        
    }
    ...
    async getExtIp(){
        try {
            if (this.GETEXTIP != null && this.GETEXTIP == false) return false;
            let extIp = null;
            if(this.accessType == 'DSL'){
                extIp = await this.soapAction(this, '/upnp/control/wanpppconn1', 'urn:dslforum-org:service:WANPPPConnection:1', 'GetInfo', null, true);
            }
            if(this.accessType == 'Ethernet'){
                extIp = await this.soapAction(this, '/upnp/control/wanipconnection1', 'urn:dslforum-org:service:WANIPConnection:1', 'GetInfo', null, true);
            }
            
            if (extIp != null && extIp.result == true){
                gthis.log.debug('external IP1: ' + JSON.stringify(extIp));
                this.adapter.setStateIfNotEqual('info.extIp', { val: extIp.resultData['NewExternalIPAddress'], ack: true });
    ...
    async getGuestWlan(id){
        try {
            ...
            if (this.WLAN3INFO && this.WLAN3INFO == true){
                guestwlan = await this.soapAction(this, '/upnp/control/wlanconfig3', urn + 'WLANConfiguration:3', 'GetInfo', null);
            ...
            if (this.WLAN3GETSECKEY && this.WLAN3GETSECKEY === true){
                const secKey = await this.soapAction(this, '/upnp/control/wlanconfig3', urn + 'WLANConfiguration:3', 'GetSecurityKeys', null);
                if (secKey && secKey['status'] == 200){
                    let password = secKey['resultData']['NewKeyPassphrase'];
                    password = password.replace(/[\\";:,]/g, '\\$&');
                    let SSID = guestwlan['resultData']['NewSSID'];
                    SSID = SSID.replace(/[\\";:,]/g, '\\$&');
                    const WPA = guestwlan['resultData']['NewBeaconType'] == '11i' ? 'WPA2' : guestwlan['resultData']['NewBeaconType'];
                    const qrcode = 'WIFI:T:' + WPA + ';P:' + password + ';S:' + SSID + ';;';
                    const svg = qr.imageSync(qrcode, { type: 'svg' });
                    //svg.pipe(require('fs').createWriteStream('/tmp/qr.png', svg));
                    //gthis.writeFileAsync('vis.0','/main/qr.png', svg);
                    gthis.setState('guest.wlanQR', { val: svg, ack: true });
                }else{
                    throw ('Can not get datamatrix code of guest wlan ' + JSON.stringify(guestwlan));                 
                }
            }
            return true;
        } catch (err) {
    ...
    async connectionCheck() {
        try {
            //const info = await this.soapAction(this, '/upnp/control/deviceinfo', 'urn:dslforum-org:service:DeviceInfo:1', 'GetInfo', null);
            const info = await this.soapAction(this, '/upnp/control/hosts', 'urn:dslforum-org:service:Hosts:1', 'GetHostNumberOfEntries', null);
    ...
    async soapAction(device, url, serviceType, action, vars, suppressMsg = false) {
        ...
        try {
            ...
            if (device._sslPort && device._auth.auth && this.adapter.config.ssl) {
            ...
            if (response.status == 200) {
                ...
                if (env['s:Body']) {
                    ...
                    if (body['u:' + action + 'Response']) {
                        ...
                        res = {'status': null, 'data': null, 'result': false, 'resultData': null, 'errNo':7 ,'errorMsg': body['s:Fault']};
                        return res;
                        //throw(body['s:Fault']);
                ...
                res = {'status': null, 'data': null, 'result': false, 'resultData': null, 'errNo':7 ,'errorMsg': 'negative response ' + JSON.stringify(response.data)};
                return res;
