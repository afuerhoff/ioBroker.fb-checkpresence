...
class Fb {
    /**
     * @param {{ host: string; uid: string; pwd: string; }} deviceInfo - configuration items
     * @param {boolean} ssl - configuration item
     * @param {ioBroker.Adapter} adapter - adapter object
     */
    constructor(deviceInfo, ssl, adapter) {
        ...
        this.WLAN4INFO = false;
        this.WLAN4GETSECKEY = false;
        this.DEVINFO = false;
     ...
     *
     * @param {{ host: string, uid: string, pwd: string }} deviceInfo  - configuration parameters for fritzbox authentification
     * @param {boolean} ssl - ssl configuration parameter
     * @param {ioBroker.Adapter} adapter - adapter object
     */
    ...
    async _checkServices() {
        try {
            ...
            this.WLAN4INFO = await this._chkService('GetInfo', 'WLANConfiguration4', 'WLANConfiguration4-GetInfo', [
                'C',
                'A',
                'P',
            ]);
            this.WLAN4GETSECKEY = await this._chkService(
                'GetSecurityKeys',
                'WLANConfiguration4',
    ...
    async getExtIpv6Prefix() {
        try {
            ...
            if (this.GETEXTIP && this.GETEXTIP == true) {
                ...
                if (soapResult) {
                    return soapResult['NewIPv6Prefix'];
                }
                const mesg = soapResult == null ? '' : soapResult;
                throw new exceptionSoapCommand(
                    'X_AVM_DE_GetIPv6Prefix',
                    `Cannot get external ipv6 prefix ${JSON.stringify(mesg)}`,
                );
            }
        } catch (error) {
            this.adapter.log.warn(`getExtIpv6Prefix: ${error.name} ${error.message}`);
            return '';
        }
    }
    ...
    async getGuestWlan() {
        try {
            let serviceType = '';
            let controlURL = '';
            if (serviceType == '' && this.WLAN4INFO && this.WLAN4INFO == true) {
                serviceType = 'urn:dslforum-org:service:WLANConfiguration:4';
                controlURL = '/upnp/control/wlanconfig4';
            }
            if (serviceType == '' && this.WLAN3INFO && this.WLAN3INFO == true) {
                serviceType = 'urn:dslforum-org:service:WLANConfiguration:3';
                controlURL = '/upnp/control/wlanconfig3';
            }
            const soapResult = await this.soapAction(controlURL, serviceType, 'GetInfo', null);
            if (soapResult) {
    ...
    async getGuestQR() {
        try {
            let serviceType = '';
            let controlURL = '';
            if (serviceType == '' && this.WLAN4GETSECKEY === true) {
                serviceType = 'urn:dslforum-org:service:WLANConfiguration:4';
                controlURL = '/upnp/control/wlanconfig4';
            }
            if (serviceType == '' && this.WLAN3GETSECKEY === true) {
                serviceType = 'urn:dslforum-org:service:WLANConfiguration:3';
                controlURL = '/upnp/control/wlanconfig3';
            }
            const soapResult = await this.soapAction(controlURL, serviceType, 'GetSecurityKeys', null);
            if (soapResult) {
                ...
                //svg.pipe(require('node:fs').createWriteStream('/tmp/qr.png', svg));
