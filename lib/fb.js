...
class Fb {
    ...
    async getMeshList(url) {
        try {
            ...
            if (response.status == 200) {
                //gthis.log.info('getMeshList ' + JSON.stringify(response.data));
                return await response.data;
            }else{
                gthis.log.error('Fb.getMeshList ' + response.data);
                return null;
            }
        } catch (error) {
            gthis.log.error('Fb.getMeshList ' + error);
            return null;
        }
    }
    ...
    async soapAction(device, url, serviceType, action, vars, suppressMsg = false) {
        ...
        try {
            ...
            if (error.response) {
                ...
                if (suppressMsg == false) gthis.log.error('soapAction ' + action + ' status=' + res.status + ' errNo=' + res.errNo + ' ' + errMsg['errorCode'] + ' ' + errMsg['errorDescription']);
