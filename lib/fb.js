    ...
    async sendWithRetry(url, timeout, start, message) {
        ...
        try {
            ...
            if (this.currentRetry < MAX_RETRY) {
                this.currentRetry++;
                gthis.log.info(message + ': Retrying ' + this.currentRetry);
                return await this.sendWithRetry(url, timeout, this.currentRetry, message);
            } else {
                this.currentRetry = 0;
                gthis.log.error(message + ': Retried ' + MAX_RETRY + ' times but still failed -> ' + JSON.stringify(error));
                return null;
            }   
        }
    }
        ...
        try {
            ...
            if (meshPath && meshPath.result == true){
                ...
                if (response && response.status == 200 && response.data) {
                    ...
                    throw ('Status = '+ response.status + ' -> Can not read meshlist path  ' + JSON.stringify(meshPath));                        
                }
            }else{
