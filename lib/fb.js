...
class Fb {
    ...
    async sendWithRetry(url, timeout, start, message) {
        let response = null;
    ...
    async getMeshList(){
        try {
            ...
            if (meshPath && meshPath.result == true){
                ...
                let response = null;
                response = await this.sendWithRetry(url, 10000, 0, 'getMeshList');
                if (response && response.status == 200 && response.data) {
    ...
    async getGuestWlan(id){
        try {
            const guestwlan = await this.soapAction(this, '/upnp/control/wlanconfig3', urn + 'WLANConfiguration:3', 'GetInfo', null);
